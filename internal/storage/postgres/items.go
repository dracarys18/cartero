...
func (s *itemStore) Exists(ctx context.Context, id string) (bool, error) {
	var exists bool
	err := s.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM items WHERE id = $1)`, id).Scan(&exists)
	if err != nil {
...
func (s *itemStore) ExistsByHash(ctx context.Context, hash string) (bool, error) {
	var exists bool
	err := s.db.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM items WHERE hash = $1)`, hash).Scan(&exists)
	if err != nil {
...
func (s *itemStore) DeleteOlderThan(ctx context.Context, age time.Duration) error {
	...
	query := `DELETE FROM items WHERE timestamp < $1`
